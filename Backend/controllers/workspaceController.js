...
export const addMember = async (req, res, next) => {
  try {
    ...
    if (!isOwner && currentMember?.role !== 'ADMIN') {
      return res
        .status(403)
        .json({ message: 'Only Admin or owner can add members' })
    }
    ...
    if (targetUser) {
      ...
      const emailInfo = buildMemberAddedEmail({
      ...
      try {
        ...
        console.error('Failed to send workspace member email:', emailError)
        emailWarning =
          'Member was added, but email could not be sent because SMTP is not configured.'
      }
    ...
    // invite external email address when user does not exist yet
